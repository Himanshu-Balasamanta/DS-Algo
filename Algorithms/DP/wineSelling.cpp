...
int maxInSubset(int year, int start, int end){
    ...
    int rMax;
    int lMax;
    if(b[start][end] != 0)return b[start][end];
    lMax = year*a[start]+maxInSubset(year + 1, start + 1, end);
    rMax = year*a[end - 1]+maxInSubset(year + 1, start, end - 1);
    b[start][end] = max(lMax,rMax);
    return b[start][end];
}
...
int main()
{
    ...
    cin>>n;
    a = vector<int>(n);
    b = vector<vector<int> >(n,vector<int>(n + 1,0));
    for(i = 0; i != n; i++)
        cin>>a[i];
